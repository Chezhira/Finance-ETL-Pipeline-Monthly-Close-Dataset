...
def build(month: str, curated_dir: Path, out_dir: Path) -> Path:
    ...
    if not merged.empty and "date" in merged.columns:
        ...
        spark_data = str(
            list(zip(dr["date"].dt.strftime("%d").tolist(), dr["amount_base"].round(0).tolist(), strict=False))
        )
    ...
    wf_data = []
    running = 0
    for lbl, val in zip(wf_labels, wf_values, strict=False):
    ...
    if not kpi_m.empty:
        ...
        for idx, (_, r) in enumerate(kpi_m.iterrows()):
    ...
    exp_rows = ""
    if not merged.empty:
        for typ, color in [("Revenue", "#4ade80"), ("Expense", "#f87171")]:
    ...
    # ── MoM banner ────────────────────────────────────────────────────────────
    mom_note = (
        "Prior month data not yet available — MoM comparison activates " "when pipeline runs across multiple months."
    )
    ...
    if not merged.empty:
        ...
        top10 = top10.nlargest(10, "abs")[
            ["date", "entity", "account_name", "account_type", "description", "amount_base"]
        ]
        src_colors = {
            "Revenue": "#4ade80",
            "Expense": "#f87171",
    ...
    # ── HTML ──────────────────────────────────────────────────────────────────
    dq_badge_bg = "#0a201580" if dq_issues == 0 else "#200a0a80"
    dq_badge_border = "#4ade8066" if dq_issues == 0 else "#f8717166"
    dq_badge_color = "#4ade80" if dq_issues == 0 else "#f87171"
    dq_badge_text = "ALL PASS" if dq_issues == 0 else "ISSUES"
    dq_kpi_color = "var(--green)" if dq_issues == 0 else "var(--red)"
    dq_kpi_text = "PASS" if dq_issues == 0 else "FAIL"
    dq_kpi_border = "var(--green)" if dq_issues == 0 else "var(--red)"
    op_border = val_color(total_op)
    gm_border = pct_color(gm_pct)
    entity_list = ", ".join(ename(e) for e in kpi_m["entity"].tolist()) if not kpi_m.empty else "—"
...
<head>
  ...
  <style>
    *,*::before,*::after{{box-sizing:border-box;margin:0;padding:0}}
    ...
    .kc{{background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:16px;animation:fadeUp .45s ease both}}
...
<div class="kpi-grid">
  ...
  <div class="kc" style="border-top:2px solid {gm_border}">
  ...
  <div class="kc" style="border-top:2px solid {op_border}">
  ...
  <div class="kc" style="border-top:2px solid {dq_kpi_border}">
    <div class="kc-label">DQ Status</div>
    <div class="kc-value" style="color:{dq_kpi_color}">{dq_kpi_text}</div>
    <div class="kc-sub">{dq_issues} issue{'s' if dq_issues != 1 else ''} flagged</div>
  </div>
</div>

<div class="mom-banner">{mom_note}</div>
...
<div class="g2">
  <div class="panel">
    <div class="ph">
      ...
      <span style="padding:2px 9px;border-radius:3px;font-size:9px;font-weight:700;letter-spacing:1px;
        background:{dq_badge_bg};border:1px solid {dq_badge_border};color:{dq_badge_color}">
        {dq_badge_text}
      </span>
    </div>
    <table>
      ...
      <tbody>
        {dq_rows or '<tr><td colspan="4" style="padding:20px 16px;text-align:center;color:#334155;font-size:11px;font-family:DM Mono,monospace">&#10003; All datasets passed quality checks</td></tr>'}
      </tbody>
    </table>
  </div>
  <div class="panel">
    ...
    <div class="pb">
      <div style="display:grid;gap:0">
        {"".join(f'<div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border)"><span style="font-size:11px;color:var(--muted)">{k}</span><span style="font-size:11px;font-family:DM Mono,monospace;color:var(--sub)">{v}</span></div>' for k, v in [("Report month", month), ("Base currency", "USD"), ("Transactions", f"{txn_count:,}"), ("Entities", entity_list), ("DQ exceptions", str(dq_issues)), ("Generated", datetime.now().strftime("%Y-%m-%d %H:%M"))])}
      </div>
    </div>
  </div>
</div>
...
(function() {{
  ...
  const pts = raw.map((r,i)=>({{x:pad.l+i*step,y:pad.t+pH*(1-(r[1]-minV)/range),v:r[1],lbl:r[0]}}));
  ...
  pts.forEach(p=>{{ ctx.beginPath(); ctx.arc(p.x,p.y,2,0,Math.PI*2); ctx.fillStyle='#4ade80'; ctx.fill(); }});
...
(function() {{
  ...
  canvas.style.width = W+'px'; canvas.style.height = H+'px';
  const ctx = canvas.getContext('2d'); ctx.scale(dpr,dpr);
  const pad = {{l:8,r:8,t:12,b:36}}, pW = W-pad.l-pad.r, pH = H-pad.t-pad.b;
  ...
  data.forEach((d,i) => {{
    const cx = pad.l+i*gap+gap*0.5, x = cx-bw*0.5;
    ...
    const color = d.isTotal?'#a78bfa':(d.value>=0?'#4ade80':'#f87171');
    if (i>0&&!d.isTotal) {{
    ...
    const v=Math.abs(d.value), lbl=v>=1000?(v/1000).toFixed(1)+'k':v.toFixed(0);
    ...
    d.label.split(' ').forEach((w,wi)=>ctx.fillText(w,cx,H-pad.b+12+wi*10));
  }});
}})();
...
(function() {{
  ...
  ctx.beginPath(); ctx.arc(cx,cy,ir,0,Math.PI*2); ctx.fillStyle='#0c1118'; ctx.fill();
  ctx.fillStyle='#e2e8f0'; ctx.font='bold 10px Playfair Display,serif'; ctx.textAlign='center';
  ctx.fillText(data.length+' Entit'+(data.length===1?'y':'ies'),cx,cy+4);
}})();
</script>
</body>
</html>"""
